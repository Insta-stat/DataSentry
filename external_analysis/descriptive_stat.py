...
df['accountName'] = df['inputUrl'].str.extract(r'instagram\.com/([^/?#]+)')

# Проверяем и исправляем отрицательные значения в ключевых метриках
negative_check_metrics = ['likesCount', 'commentsCount', 'videoPlayCount']
for metric in negative_check_metrics:
...
df['likeCommentRate'] = df['commentsCount'] / df['likesCount']
